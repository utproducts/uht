...
registrationRoutes.get('/all', authMiddleware, requireRole('admin', 'director'), async (c) => {
  ...
  let q1 = `
    SELECT r.*,
      ...
      t.age_group as team_age_group, t.division_level as team_division_level, t.city as team_city, t.state as team_state,
      t.usa_hockey_roster_url, t.mhr_url,
      ...
      e.name as event_name, COALESCE(e.is_test,0) as event_is_test, e.city as event_city, e.state as event_state, e.start_date as event_start_date,
  ...
  let q2 = `
    SELECT er.*,
      er.team_name, ct.schedule_name, (SELECT og.name FROM organizations og WHERE og.id = ct.organization_id) as org_name, er.age_group as team_age_group, COALESCE(ct.division_level, er.division) as team_division_level,
      CASE WHEN er.team_id IS NOT NULL THEN ct.usa_hockey_roster_url ELSE (SELECT t2.usa_hockey_roster_url FROM teams t2 WHERE LOWER(t2.name) = LOWER(er.team_name) AND t2.usa_hockey_roster_url IS NOT NULL AND t2.usa_hockey_roster_url != '' LIMIT 1) END as usa_hockey_roster_url,
      CASE WHEN er.team_id IS NOT NULL THEN ct.mhr_url ELSE (SELECT t3.mhr_url FROM teams t3 WHERE LOWER(t3.name) = LOWER(er.team_name) AND t3.mhr_url IS NOT NULL AND t3.mhr_url != '' LIMIT 1) END as mhr_url,
