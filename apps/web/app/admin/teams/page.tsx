...
function EditTeamModal({ team, onClose, onSaved }: { team: Team; onClose: () => void; onSaved: () => void }) {
  ...
  const fc = "w-full px-3 py-2 border border-[#e8e8ed] rounded-xl text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none";
  const lc = "block text-xs font-semibold text-[#6e6e73] mb-1";

  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="border-b border-[#e8e8ed] px-6 py-4 flex items-center justify-between">
          <h2 className="text-lg font-bold text-[#1d1d1f]">Edit Team</h2>
  ...
  return (
    <div className="bg-[#fafafa] min-h-full">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-[#1d1d1f] mb-1">Deactivate Team</h3>
              <p className="text-sm text-[#86868b] mb-4">Are you sure you want to deactivate <strong>{deleteConfirm.name}</strong>?</p>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-6 pb-2 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-[#1d1d1f]">Team Management</h1>
          <p className="text-sm text-[#86868b] mt-0.5">{teams.length.toLocaleString()} teams in database</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-2">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-white rounded-xl shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-[#1d1d1f]">{teams.length.toLocaleString()}</div>
            <div className="text-xs text-[#86868b] mt-1">Total Teams</div>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-[#003e79]">{teams.filter(t => t.age_group !== 'Unknown').length.toLocaleString()}</div>
            <div className="text-xs text-[#86868b] mt-1">Categorized</div>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-amber-600">{teams.filter(t => t.age_group === 'Unknown').length.toLocaleString()}</div>
            <div className="text-xs text-[#86868b] mt-1">Needs Review</div>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-4 text-center">
            <div className="text-2xl font-bold text-green-600">{teams.filter(t => t.player_count > 0).length}</div>
            <div className="text-xs text-[#86868b] mt-1">With Roster</div>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex-1 max-w-sm">
            <input
              ...
              className="w-full px-4 py-2 border border-[#e8e8ed] rounded-xl text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-2 border border-[#e8e8ed] rounded-xl text-sm bg-white focus:ring-2 focus:ring-cyan-500 outline-none"
          >
          ...
          <span className="text-sm text-[#86868b]">{filtered.length.toLocaleString()} results</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-5">
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <p className="text-[#86868b] font-medium">No teams found</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#e8e8ed] bg-[#f5f5f7]/50">
                    <th className="text-left px-5 py-3 text-xs font-bold text-[#86868b] uppercase tracking-wide">Team Name</th>
                    <th className="text-left px-5 py-3 text-xs font-bold text-[#86868b] uppercase tracking-wide">Age Group</th>
                    <th className="text-left px-5 py-3 text-xs font-bold text-[#86868b] uppercase tracking-wide">Division</th>
                    <th className="text-left px-5 py-3 text-xs font-bold text-[#86868b] uppercase tracking-wide">Location</th>
                    <th className="text-center px-5 py-3 text-xs font-bold text-[#86868b] uppercase tracking-wide">Players</th>
                    <th className="text-right px-5 py-3 text-xs font-bold text-[#86868b] uppercase tracking-wide">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#e8e8ed]">
                  {paginated.map(team => (
                    <tr key={team.id} className="hover:bg-[#f5f5f7]/50 transition">
                      <td className="px-5 py-3">
                        <div className="font-semibold text-sm text-[#1d1d1f]">{team.name}</div>
                        {team.organization_name && <div className="text-xs text-[#86868b] mt-0.5">{team.organization_name}</div>}
                      </td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3 text-sm text-[#6e6e73]">{team.division_level || '—'}</td>
                      <td className="px-5 py-3 text-sm text-[#6e6e73]">
                        {team.city && team.state ? `${team.city}, ${team.state}` : team.state || '—'}
                      </td>
                      <td className="px-5 py-3 text-center">
                        <span className="text-sm font-bold text-[#1d1d1f]">{team.player_count}</span>
                      </td>
                      <td className="px-5 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setEditingTeam(team)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-blue-50 text-[#86868b] hover:text-[#003e79] transition" title="Edit">
                          ...
                          <button onClick={() => setDeleteConfirm(team)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-[#86868b] hover:text-red-600 transition" title="Deactivate">
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg text-sm font-medium transition ${
                          page === p ? 'bg-cyan-600 text-white shadow' : 'bg-white border border-[#e8e8ed] hover:bg-[#f5f5f7] text-[#3d3d3d]'
                        }`}
                      >
                        {p}
                      </button>
