...
import { webUpdateRoutes } from './routes/web-updates';
...
app.route('/api/web-updates', webUpdateRoutes);
...
// Send video follow-up email to RSVP'd coaches
app.post('/api/coaches-meeting/send-video', async (c) => {
  const body = await c.req.json() as { sendKey?: string; test?: boolean; testEmail?: string };
  ...
  let recipients = (rsvps.results || []).filter((r: any) => r.email);

  if (body.test && body.testEmail) {
    recipients = [{ email: body.testEmail, name: 'Test' }];
  }
  ...
  const subject = "In Case You Missed It — Coaches Meeting Recording";
...
<tr><td style="padding:32px 28px;">
  <p style="color:#1d1d1f;font-size:18px;line-height:1.6;margin:0 0 16px;font-weight:600;">
    In case you missed it!
  </p>

  <p style="color:#6e6e73;font-size:15px;line-height:1.7;margin:0 0 24px;">
    Thanks for RSVPing to our Coaches &amp; Managers Meeting. Whether you were there live or couldn't make it, the full recording is now available for you to watch at your convenience.
  </p>

  <div style="text-align:center;margin:28px 0;">
    <a href="https://ultimatetournaments.com/coaches-meeting/" style="display:inline-block;background-color:#003e79;color:#ffffff;padding:18px 48px;border-radius:28px;text-decoration:none;font-size:18px;font-weight:bold;letter-spacing:0.3px;">Watch the Recording</a>
  </div>

  <p style="color:#6e6e73;font-size:13px;line-height:1.6;margin:16px 0 0;text-align:center;">
    Or visit: <a href="https://ultimatetournaments.com/coaches-meeting/" style="color:#003e79;">ultimatetournaments.com/coaches-meeting</a>
  </p>
</td></tr>
