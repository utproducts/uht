...
registrationRoutes.get('/all', authMiddleware, requireRole('admin', 'director'), async (c) => {
  ...
  let q2 = `
    SELECT er.*,
      ...
      COALESCE(ct.usa_hockey_roster_url, (SELECT t2.usa_hockey_roster_url FROM teams t2 WHERE LOWER(t2.name) = LOWER(er.team_name) AND t2.usa_hockey_roster_url IS NOT NULL AND t2.usa_hockey_roster_url != '' LIMIT 1)) as usa_hockey_roster_url,
      COALESCE(ct.mhr_url, (SELECT t3.mhr_url FROM teams t3 WHERE LOWER(t3.name) = LOWER(er.team_name) AND t3.mhr_url IS NOT NULL AND t3.mhr_url != '' LIMIT 1)) as mhr_url,
