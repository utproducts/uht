...
schedulingRoutes.put('/games/:gameId', authMiddleware, requireRole('admin', 'director'), zValidator('json', updateGameSchema), async (c) => {
  ...
  const updated = await db.prepare(`
    SELECT g.*, COALESCE(ht.schedule_name, CASE WHEN ht.head_coach_name LIKE '% %' THEN COALESCE((SELECT og.name FROM organizations og WHERE og.id = ht.organization_id), ht.name) || ' (' || TRIM(SUBSTR(ht.head_coach_name, INSTR(ht.head_coach_name, ' '))) || ')' ELSE ht.name END) as home_team_name, COALESCE(at.schedule_name, CASE WHEN at.head_coach_name LIKE '% %' THEN COALESCE((SELECT og.name FROM organizations og WHERE og.id = at.organization_id), at.name) || ' (' || TRIM(SUBSTR(at.head_coach_name, INSTR(at.head_coach_name, ' '))) || ')' ELSE at.name END) as away_team_name, vr.name as rink_name, v.name as venue_name,
